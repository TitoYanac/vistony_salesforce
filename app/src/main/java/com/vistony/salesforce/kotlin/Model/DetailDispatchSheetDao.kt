...
interface DetailDispatchSheetDao {
    ...
    @Query(
        "Select DISTINCT  A.id,A.compania_id,A.fuerzatrabajo_id,A.usuario_id,A.control_id,A.item_id,A.cliente_id,A.domembarque_id,A.direccion,A.factura_id,A.entrega_id,A.entrega,A.factura " +
                ...
                ",fotoguia AS fotoguia,fotolocal AS fotolocal,C.chk_statusServerDispatch as chk_statusServerDispatch,D.driverName as driverName,D.driverMobile as driverMobile,C.messageServerDispatch AS messageServerDispatch " +
                " from detaildispatchsheet A" +
                " left outer join cliente B ON  " +
                "A.cliente_id=B.cliente_id " +
                " left outer join (SELECT control_id,fechahojadespacho,fuerzatrabajo_id,driverName,driverMobile FROM headerdispatchsheet group by control_id,fechahojadespacho,fuerzatrabajo_id,driverName,driverMobile ) D ON  " +
                "A.fuerzatrabajo_id=D.fuerzatrabajo_id  AND " +
                "A.control_id=D.control_id   " +
                " left outer join (SELECT compania_id,cliente_id,entrega_id,fuerzatrabajo_id,DocEntry as control_id,LineId as item_id,chk_statusServerDispatch,messageServerDispatch FROM statusdispatch group by compania_id,cliente_id,entrega_id,fuerzatrabajo_id,DocEntry,LineId,chk_statusServerDispatch,messageServerDispatch ) C ON  " +
    ...
    @Query(
        "Select DISTINCT " +
                "A.id,A.compania_id,A.fuerzatrabajo_id,A.usuario_id,A.control_id,A.item_id,A.cliente_id,A.domembarque_id,A.direccion,A.factura_id,A.entrega_id,A.entrega,A.factura " +
                ",A.saldo,G.typedispatch as estado,A.factura_fuerzatrabajo_id,A.factura_fuerzatrabajo,A.terminopago_id,A.terminopago " +
                ",A.peso" +
                ",G.typedispatch,H.reasondispatch as motivo,IFNULL(e.latitudfin,'0') as latitud, IFNULL(e.longitudfin,'0') as longitud " +
                ",A.comentariodespacho,B.nombrecliente" +
                ",IFNULL(E.timeini,A.timeini) timeini, IFNULL(A.estado,IFNULL(E.timefin,A.timefin)) timefin" +
                ",(CASE WHEN E.timeini IS NOT NULL THEN 'Y' ELSE 'N' END) AS statusvisitstart" +
                ",(CASE WHEN c.compania_id IS NOT NULL  THEN 'Y' ELSE 'N' END) AS statusupdatedispatch" +
                ...
                "a.cliente_id=I.cliente_id and " +
                "a.domembarque_id=I.domembarque_id  " +
                " where D.fechahojadespacho= :dateDispatch and E.timefin<>'0' " +
                //" AND G.typedispatch_id in :statusDispatch " +
                " ORDER BY E.timefin"
    )
    fun getDetailDispatchSheetforMap(
        dateDispatch: String,
    ): List<DetailDispatchSheetUI>?
