...
interface DetailDispatchSheetDao {
    ...
    @Query(
        "Select DISTINCT A.id,A.compania_id,A.fuerzatrabajo_id,A.usuario_id,A.control_id,A.item_id,A.cliente_id,A.domembarque_id,A.direccion,A.factura_id,A.entrega_id,A.entrega,A.factura " +
                ...
                ",(CASE WHEN E.timeini IS NOT NULL OR  A.timefin  IS NOT NULL THEN 'Y' ELSE 'N' END) AS statusvisitstart" +
                ",(CASE WHEN c.compania_id IS NOT NULL OR  A.estado_id <> 'P' THEN 'Y' ELSE 'N' END) AS statusupdatedispatch" +
                ",(CASE WHEN F.DocEntryFT IS NOT NULL OR  A.saldo > 0 THEN 'Y' ELSE 'N' END) AS statuscollection" +
                ",(CASE WHEN E.timefin IS NOT NULL OR  A.timefin IS NOT NULL THEN 'Y' ELSE 'N' END) AS  statusvisitend" +
    ...
    fun getDetailDispatchSheetforDispatchDatePending(dateDispatch: String
    ...
    fun getDetailDispatchSheetforDispatchDateFailed(dateDispatch: String
        //,statusDispatch: String
    ): List<DetailDispatchSheet>?
    ...
    fun getDetailDispatchSheetforDispatchDateSucessful(dateDispatch: String
        //,statusDispatch: String
    ): List<DetailDispatchSheet>?
}