...
public class ReasonDispatchSQLite {
    ...
    public ReasonDispatchSQLite(Context context)
    {
    ...
    public int AddReasonDispatch (List<ReasonDispatchEntity> listReasonDispatchEntity)
    {
        abrir();
        for (int i = 0; i < listReasonDispatchEntity.size(); i++) {
            ...
            registro.put("reasondispatch_id",listReasonDispatchEntity.get(i).getReasondispatch_id());
            registro.put("reasondispatch",listReasonDispatchEntity.get(i).getReasondispatch());
            registro.put("typedispatch_id",listReasonDispatchEntity.get(i).getTypedispatch_id());
            bd.insert("reasondispatch",null,registro);
        }
        bd.close();
        return 1;
    }
    ...
    public ArrayList<ReasonDispatchEntity> getReasonDispatchForType (String typedispatch_id)
    {
        ...
        Cursor fila = bd.rawQuery(
                "Select * from reasondispatch where typedispatch_id='"+typedispatch_id+"'",null);
    ...
    public int DeleteTableReasonDispatch ()
    {
        ...
        bd.execSQL("delete from reasondispatch");
        bd.close();
        //Toast.makeText(this,"Ss cargaron los datos del articulo", Toast.LENGTH_SHORT).show();
        return 1;
    }

    public Integer getCountReasonDispatch ()
    {
        Integer resultado=0;

        try {
            abrir();
            Cursor fila = bd.rawQuery("Select count(compania_id) from reasondispatch",null);
